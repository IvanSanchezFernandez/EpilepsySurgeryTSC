...
# Combined surgeries
combined_surgeries <- TSC_refractory_epilepsy %>% 
  select(lobectomy, hemispherectomy, other_excision_or_disconnection_or_destruction, LITT, intracranial_neurostimulator) %>% 
  rowwise() %>% 
  mutate(yes_count = sum(c_across(everything()) == "yes")) %>% 
  filter(yes_count > 1) %>% 
  ungroup()
```
